...
win.geometry("550x290")#Laki ng window x,y sya

#eto ung student number
StudNO=Label(win, text="Student Number" ,fg="yellow"  ,bg="#674ea7" ,font=('times', 15, ' bold ') )
StudNO.place(x=20, y=150)
NOin=Entry(win,width=30  ,bg="#cccccc" ,fg="black",font=('times', 15, ' bold '))
NOin.place(x=180, y=150)

#Eto code ng input ID
enterID=Label(win, text="Enter ID Number" ,fg="yellow"  ,bg="#674ea7" ,font=('times', 15, ' bold ') ) 
enterID.place(x=20, y=90)
IDin=Entry(win,width=30  ,bg="#cccccc" ,fg="black",font=('times', 15, ' bold '))
IDin.place(x=180, y=90)


#Eto naman sa input Name igeget sya sa baba
enterNAME=Label(win, text="Enter NAME" ,fg="yellow"  ,bg="#674ea7" ,font=('times', 15, ' bold ') ) 
enterNAME.place(x=20, y=30)
NAMEin=Entry(win,width=30  ,bg="#cccccc" ,fg="black",font=('times', 15, ' bold '))
NAMEin.place(x=180, y=30)
...
def capture():
    ...
    def insertOrUpdate(ID, Name, StudNumber):   #Eto function para maconnect sa database SQLITE3
...
def addStudNo():
   msg = messagebox.showinfo( "Add Student Number", 'Put your name inside a quotation mark like this:\n\t           "Your Name" ')
...
howaddID.place(x = 495, y = 30)

howaddname = Button(win, text = " ? ", font= 'Arial 12 bold', command = addname, bg='#a9a9a9', fg='black')
howaddname.place(x = 495, y = 90)

howaddname = Button(win, text = " ? ", font= 'Arial 12 bold', command = addname, bg='#a9a9a9', fg='black')
howaddname.place(x = 495, y = 150)
